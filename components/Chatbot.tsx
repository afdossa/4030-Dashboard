...
const functionDeclarations = [
    {
        name: 'resizeChart',
        description: 'Adjusts the size of any chart (bar, bar2, line, scatter) in the dashboard.',
        parameters: {
            type: 'OBJECT',
            properties: {
                chartName: { type: 'STRING', description: 'The internal chart key: "bar", "bar2", "line", or "scatter".' },
                size: { type: 'STRING', description: 'The desired size: "small", "medium", "large", or "full".' },
            },
    ...
    {
        name: 'applyFilters',
        description: 'Filters the dashboard data based on specific towns or property types.',
        parameters: {
            type: 'OBJECT',
            properties: {
                towns: { type: 'ARRAY', items: { type: 'STRING' }, description: 'List of towns to filter by (e.g., ["Ridgefield", "Danbury"]). Use an empty array to clear town filters.' },
                propertyTypes: { type: 'ARRAY', items: { type: 'STRING' }, description: 'List of property types to filter by (e.g., ["Residential", "Commercial"]). Use an empty array to clear property type filters.' },
            },
            required: [],
        },
    },
    {
        ...
        description: 'Calculates the average, sum, count, or median of a data field, optionally grouped by town or property type.',
        parameters: {
            type: 'OBJECT',
            properties: {
                field: { type: 'STRING', description: 'The data field to calculate on. E.g., "sale_amount", "assessed_value", "list_year".' },
                operation: { type: 'STRING', description: 'The aggregation operation: "average", "sum", "count", or "median".' },
                groupBy: { type: 'STRING', description: 'Optional field to group the calculation by, e.g., "town" or "property_type".' },
            },
    ...
    },
    {
        ...
        description: 'Updates the data field (Y-axis) plotted on the main bar charts and sets the new chart title. Use this when the user asks to change what a chart shows.',
        parameters: {
            type: 'OBJECT',
            properties: {
                chartName: {
                    type: 'STRING',
                    description: "The chart key to modify. Must be one of 'bar' (Town Chart) or 'bar2' (Property Type Chart).",
                },
                field: {
                    type: 'STRING',
                    description: "The aggregate data field to plot on the Y-axis. Must be one of: 'sales_count', 'avg_sale_amount', 'total_sale_amount', or 'avg_assessed_value'.",
                },
                title: {
                    type: 'STRING',
...
const Chatbot: React.FC<ChatbotProps> = ({ onFunctionCall, salesData }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await runChat(input, salesData, functionDeclarations);
